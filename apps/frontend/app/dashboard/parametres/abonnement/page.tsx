  ...
  return (
    <div className="space-y-6 bg-[#f9fafd] p-6 rounded-3xl min-h-screen text-slate-800 relative">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-900">{du("m0de896fd60f8")}</h1>
          <p className="text-xs text-slate-400 font-medium mt-1">{du("m89cd4e42a3db")}</p>
        </div>
      ...
      <section className="bg-slate-950 rounded-3xl border border-slate-800 p-6 text-white overflow-hidden relative">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-5">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-300">{du("mff34b85c0fc0")}</p>
            <h2 className="text-3xl font-black mt-2">{subscription?.planName || du("mba6482c79ca4")}</h2>
            <p className="text-xs text-slate-400 mt-2">{du("m1eb543dc320e")}{" "}{subscription?.status || du("m240ffa1f9fc5")}</p>
          </div>
          <div className="grid grid-cols-3 gap-3 min-w-[260px]">
            <div className="bg-white/8 border border-white/10 rounded-2xl p-3">
              <Store size={16} className="text-indigo-300" />
              <p className="text-[10px] font-bold text-slate-400 mt-2">{du("m3cdc828d71de")}</p>
              <p className="text-sm font-black">{plans.find((p) => p.code === subscription?.planCode)?.limits.boutiques || 1}</p>
            </div>
            <div className="bg-white/8 border border-white/10 rounded-2xl p-3">
              <Users size={16} className="text-indigo-300" />
              <p className="text-[10px] font-bold text-slate-400 mt-2">{du("m23d3938b5ef0")}</p>
            ...
            <div className="bg-white/8 border border-white/10 rounded-2xl p-3">
              ...
              <p className="text-[10px] font-bold text-slate-400 mt-2">{du("m49b3cf78ac70")}</p>
      ...
      <section className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-4 flex flex-col md:flex-row md:items-end gap-3">
        <label className="flex-1 space-y-1.5">
          <span className="text-[10px] font-black uppercase tracking-wider text-slate-400">{du("m7ea531dc97e5")}</span>
          <input value={paymentPhone} onChange={(event) => setPaymentPhone(event.target.value)} className="w-full h-11 rounded-xl border border-slate-200 px-4 text-sm font-bold text-slate-700 outline-none focus:border-indigo-500" placeholder={du("mcfc06dde9f59")} />
        </label>
        <p className="text-[11px] text-slate-400 font-semibold md:max-w-md">{du("m7abaf02d954a")}</p>
      </section>
      ...
      {pendingPayment && (
        <section className="bg-amber-50 rounded-2xl border border-amber-100 shadow-sm p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
          <div>
            <p className="text-xs font-black text-amber-800 uppercase tracking-wider">{du("ma7e2d23cf13c")}</p>
            <p className="text-[11px] text-amber-700 mt-1">{du("m3e152bf81a36")}{" "}<span className="font-black">{pendingPayment.orderNumber}</span>{du("maf0481d2172e")}</p>
          </div>
            ...
            return (
              <motion.article key={plan.code} whileHover={{ y: -4 }} className={`bg-white rounded-2xl border shadow-sm p-5 flex flex-col ${active ? "border-indigo-300 ring-4 ring-indigo-50" : "border-slate-200/80"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <h2 className="text-sm font-black text-slate-950">{du(plan.name)}</h2>
                    <p className="text-[11px] text-slate-500 mt-1 leading-relaxed">{du(plan.description)}</p>
                  </div>
                  {active ? <span className="px-2 py-1 rounded-lg bg-indigo-50 text-indigo-600 text-[9px] font-black">{du("mf2ccbaf504ce")}</span> : <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-50 border border-slate-200 text-slate-500 text-[9px] font-black uppercase"><LockKeyhole size={11} /> {du("m7ec0261b8e8a")}</span>}
                </div>

                <div className="mt-5">
                  <span className="text-3xl font-black text-slate-950">{plan.priceMonthly === 0 ? "0" : plan.priceMonthly}</span>
                  <span className="text-xs font-bold text-slate-400"> {plan.currency}{du("m67e11cc52e88")}</span>
                </div>
                ...
                <div className="mt-5 space-y-2 flex-1">
                  {plan.features.slice(0, 7).map((feature) => (
                    <div key={feature} className="flex items-center gap-2 text-[10px] font-bold text-slate-600">
                      ...
                      {du(feature.replaceAll("_", " ").toLowerCase())}
                    </div>
                  ))}
                  {plan.features.length > 7 && <p className="text-[10px] font-black text-indigo-600">+{plan.features.length - 7} {du("mb4d4237e9025")}</p>}
                </div>
                ...
                <div className="mt-6 space-y-2">
                  ...
                  >
                    ...
                    {active ? du("mff34b85c0fc0") : plan.code === "TRIAL" ? du("m67f4b1e7df21") : du("m2c128bd05cd8")}
                  </button>
