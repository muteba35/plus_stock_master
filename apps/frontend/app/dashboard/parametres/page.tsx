...
export default function GeneralSettingsPage() {
  ...
  return <div className="space-y-6 bg-[#f9fafd] p-6 rounded-3xl min-h-screen text-slate-800">
    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      <div>
        <h1 className="text-xl font-bold text-slate-900">{du("m5744469bace9")}</h1>
        <p className="text-xs text-slate-400 font-medium mt-1">{du("ma065d664c13f")}</p>
      </div>
    ...
    <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
      {sections.map((section) => {
        ...
        return <Link key={section.href} href={section.href} className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 hover:-translate-y-0.5 hover:shadow-md transition-all group">
          ...
          <h2 className="mt-5 text-sm font-black text-slate-900">{du(section.title)}</h2>
          <p className="mt-2 text-xs leading-relaxed text-slate-500">{du(section.text)}</p>
        </Link>;
      })}
    </section>
    ...
    <section className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
      <div className="p-5 border-b border-slate-100">
        <h2 className="text-sm font-bold text-slate-900">{du("me772fd8ce0e7")}</h2>
        <p className="text-[11px] text-slate-400 mt-1">{du("m8fd9a6b1aee3")}</p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-slate-100">
        {rules.map((rule) => {
          ...
          return <div key={rule.title} className="p-5">
            ...
            <h3 className="mt-4 text-xs font-black uppercase tracking-wider text-slate-800">{du(rule.title)}</h3>
            <p className="mt-2 text-xs leading-relaxed text-slate-500">{du(rule.text)}</p>
          </div>;
        })}
      </div>
    </section>
