...
export default function ProductImportModal({ open, saving, rows, fileName, error, currency, onClose, onDownloadTemplate, onFile, onImport }: Props) {
  return (
    <InventoryModal open={open} onClose={onClose} title="Importer plusieurs produits" subtitle="Import CSV compatible Excel, détail ou gros, limité à 500 lignes."
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-xl bg-indigo-50 border border-indigo-100">
          <div><p className="text-xs font-bold text-indigo-800">Modèle Excel obligatoire</p><p className="text-[10px] text-indigo-600 mt-1">Les catégories doivent déjà exister. Pour le gros, renseignez le nombre de conditionnements et les unités par conditionnement.</p></div>
        ...
        {rows.length > 0 && <div className="border border-slate-200 rounded-xl overflow-hidden"><div className="px-3 py-2 bg-slate-50 text-[10px] font-bold uppercase text-slate-400">Aperçu des premières lignes</div><div className="divide-y divide-slate-100">{rows.slice(0, 4).map((row) => <div key={row.line} className="px-3 py-2 flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold text-slate-700 truncate">{row.nom}</p><p className="text-[10px] text-slate-400 truncate">{row.sku} · {row.categorie} · {row.modeApprovisionnement === "GROS" ? `${row.nombreConditionnements || 0} x ${row.quantiteParConditionnement || 0}` : "Détail"}</p></div><span className="text-xs font-bold text-slate-600 whitespace-nowrap">{formatMoney(row.prixVente, row.devise || currency)}</span></div>)}</div></div>}
      </div>
    </InventoryModal>
  );
}
