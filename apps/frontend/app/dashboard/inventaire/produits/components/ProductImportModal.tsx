...
type Props = {
  open: boolean; saving: boolean; rows: ImportRow[]; fileName: string; error: string;
  onClose: () => void; onDownloadTemplate: () => void; onFile: (file?: File) => void; onImport: () => void;
};

export default function ProductImportModal({ open, saving, rows, fileName, error, onClose, onDownloadTemplate, onFile, onImport }: Props) {
  return (
    <InventoryModal open={open} onClose={onClose} title="Importer plusieurs produits" subtitle="Import CSV compatible Excel, limité à 500 lignes."
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-xl bg-indigo-50 border border-indigo-100">
          <div><p className="text-xs font-bold text-indigo-800">Modèle Excel obligatoire</p><p className="text-[10px] text-indigo-600 mt-1">Les catégories indiquées doivent déjà exister dans la boutique.</p></div>
          <button type="button" onClick={onDownloadTemplate} className={`${secondaryButton} border-indigo-200 text-indigo-700`}><Download size={14} /> Télécharger le modèle</button>
        </div>
        ...
        {rows.length > 0 && <div className="border border-slate-200 rounded-xl overflow-hidden"><div className="px-3 py-2 bg-slate-50 text-[10px] font-bold uppercase text-slate-400">Aperçu des premières lignes</div><div className="divide-y divide-slate-100">{rows.slice(0, 4).map((row) => <div key={row.line} className="px-3 py-2 flex items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs font-bold text-slate-700 truncate">{row.nom}</p><p className="text-[10px] text-slate-400 truncate">{row.sku} · {row.categorie}</p></div><span className="text-xs font-bold text-slate-600 whitespace-nowrap">{Number(row.prixVente).toLocaleString("fr-FR")} $</span></div>)}</div></div>}
      </div>
    </InventoryModal>
  );
}
