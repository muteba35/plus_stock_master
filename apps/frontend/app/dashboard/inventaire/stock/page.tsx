...
export default function MouvementsStockPage() {
  ...
  return (
    <div className="space-y-6 bg-[#f9fafd] p-3 sm:p-6 rounded-2xl sm:rounded-3xl min-h-screen text-slate-800 overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><MetricCard label="Entrées ce mois" value={compactNumber(summary.entries)} detail="Unités réceptionnées" icon={ArrowDownLeft} tone="emerald" onInspect={() => setMetricDetail({ title: "Entrées ce mois", value: summary.entries.toLocaleString("fr-FR"), detail: "Unités réceptionnées" })} /><MetricCard label="Sorties ce mois" value={compactNumber(summary.exits)} detail="Sorties manuelles enregistrées" icon={ArrowUpRight} tone="rose" onInspect={() => setMetricDetail({ title: "Sorties ce mois", value: summary.exits.toLocaleString("fr-FR"), detail: "Sorties manuelles enregistrées" })} /><MetricCard label="Ajustements" value={compactNumber(summary.adjustments)} detail="Comptages corrigés ce mois" icon={ArrowLeftRight} tone="amber" onInspect={() => setMetricDetail({ title: "Ajustements", value: summary.adjustments.toLocaleString("fr-FR"), detail: "Comptages corrigés ce mois" })} /></div>
      ...
      <div className={`bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-visible ${activeView === "movements" ? "block" : "hidden"}`}>
        <div className="relative z-30 p-4 border-b border-slate-100 flex gap-2 bg-white rounded-t-2xl"><SearchInput value={search} onChange={setSearch} placeholder="Rechercher un produit, SKU, motif ou référence..." /><div className="relative" ref={filterRef}><button onClick={() => setFilterOpen((current) => !current)} className={`h-10 w-10 rounded-xl border flex items-center justify-center relative ${filterOpen || activeFilterCount ? "border-indigo-300 bg-indigo-50 text-indigo-600" : "border-slate-200 text-slate-500"}`} title="Filtrer"><SlidersHorizontal size={16} />{activeFilterCount > 0 && <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-indigo-600 text-white text-[9px] font-bold flex items-center justify-center">{activeFilterCount}</span>}</button>{filterOpen && <div className="absolute right-0 top-12 z-[70] w-[min(20rem,calc(100vw-2rem))] bg-white border border-slate-200 rounded-xl shadow-[0_18px_45px_-12px_rgba(15,23,42,0.25)] p-4 space-y-4"><div className="flex justify-between"><p className="text-xs font-bold">Filtres</p><button onClick={resetFilters} className="text-[10px] font-bold text-indigo-600 flex items-center gap-1"><RotateCcw size={11} /> Réinitialiser</button></div><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase text-slate-400">Type</span><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)} className={fieldClass}><option value="all">Tous</option><option value="ENTREE">Entrées</option><option value="SORTIE">Sorties</option><option value="AJUSTEMENT">Ajustements</option></select></label><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase text-slate-400">Période rapide</span><select value={periodFilter} onChange={(event) => setPeriodFilter(event.target.value)} className={fieldClass}><option value="all">Toutes les dates</option><option value="today">Aujourd'hui</option><option value="month">Ce mois</option></select></label><div className="grid grid-cols-2 gap-3"><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase text-slate-400">Du</span><input type="date" value={dateFrom} max={dateTo || undefined} onChange={(event) => { setDateFrom(event.target.value); setPeriodFilter("all"); }} className={fieldClass} /></label><label className="block space-y-1.5"><span className="text-[10px] font-bold uppercase text-slate-400">Au</span><input type="date" value={dateTo} min={dateFrom || undefined} onChange={(event) => { setDateTo(event.target.value); setPeriodFilter("all"); }} className={fieldClass} /></label></div><button onClick={() => setFilterOpen(false)} className={`${primaryButton} w-full`}>Appliquer</button></div>}</div></div>
