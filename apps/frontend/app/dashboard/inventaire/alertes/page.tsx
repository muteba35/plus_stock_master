...
export default function AlertesStockPage() {
  ...
  return (
    <div className="space-y-6 bg-[#f9fafd] p-3 sm:p-6 rounded-2xl sm:rounded-3xl min-h-screen text-slate-800 overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4"><MetricCard label="Alertes actives" value={compactNumber(summary.total)} detail="Produits à surveiller" icon={BellRing} tone="amber" onInspect={() => setMetricDetail({ title: "Alertes actives", value: summary.total.toLocaleString("fr-FR"), detail: "Produits à surveiller" })} /><MetricCard label="Ruptures" value={compactNumber(summary.ruptures)} detail="Action immédiate requise" icon={AlertOctagon} tone="rose" onInspect={() => setMetricDetail({ title: "Ruptures", value: summary.ruptures.toLocaleString("fr-FR"), detail: "Produits en rupture" })} /><MetricCard label="Stock faible" value={compactNumber(summary.lowStock)} detail="Sous le seuil minimum" icon={AlertTriangle} tone="amber" onInspect={() => setMetricDetail({ title: "Stock faible", value: summary.lowStock.toLocaleString("fr-FR"), detail: "Produits sous le seuil minimum" })} /></div>
