  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="min-w-0">
        <h1 className="text-xl font-bold text-slate-900">{du(title)}</h1>
        <p className="text-xs text-slate-400 font-medium mt-1">{du(subtitle)}</p>
      </div>
      {action}
    </div>
  );
...
export function StatusBadge({ status }: { status: ProductStatus }) {
  ...
  return <span className={`inline-flex px-2 py-1 rounded-md text-[10px] font-bold whitespace-nowrap ${style}`}>{du(status)}</span>;
}
...
export function InventoryPagination({ page, pageSize, totalItems, onPageChange }: { page: number; pageSize: number; totalItems: number; onPageChange: (page: number) => void }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-3 border-t border-slate-100 bg-white rounded-b-2xl">
      <span className="text-[10px] font-medium text-slate-400">{du("m0a30a815d67d")}{" "}{page} {du("mc89b6c3d164a")}{" "}{totalPages}</span>
      <div className="flex items-center gap-1">
        <button type="button" disabled={page <= 1} onClick={() => onPageChange(page - 1)} className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50 disabled:opacity-30 disabled:cursor-not-allowed" title={du("m0301d410db18")}><ChevronLeft size={15} /></button>
        ...
        <button type="button" disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-slate-50 disabled:opacity-30 disabled:cursor-not-allowed" title={du("m10733e78ff61")}><ChevronRight size={15} /></button>
      </div>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[100] bg-slate-950/45 backdrop-blur-[2px] p-3 sm:p-6 flex items-center justify-center" onMouseDown={onClose}>
      <div className="w-full max-w-xl max-h-[92dvh] bg-white rounded-2xl border border-slate-200 shadow-2xl overflow-hidden flex flex-col" onMouseDown={(event) => event.stopPropagation()}>
        <div className="p-5 border-b border-slate-100 flex items-center justify-between gap-4 bg-slate-50/50">
          <div><h2 className="text-sm font-bold text-slate-900 uppercase tracking-wide">{du(title)}</h2><p className="text-[11px] text-slate-400 mt-1">{du(subtitle)}</p></div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:bg-slate-100" title={du("m711e5f2e198d")}><X size={16} /></button>
        </div>
        {notice && <div className="px-5 pt-4 bg-white">{du(notice)}</div>}
