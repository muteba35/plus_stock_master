...
export default function InventoryProjectionPage() {
  ...
  const exportPdf = () => {
    ...
    if (!popup) return;
    const rows = filteredRows.slice(0, 80).map((row) => "<tr><td>" + escapeHtml(row.produit) + "</td><td>" + escapeHtml(row.categorie) + "</td><td>" + escapeHtml(row.quantiteActuelle) + "</td><td>" + escapeHtml(formatMoney(row.prixAchatUnitaire, data.devise)) + "</td><td>" + escapeHtml(formatMoney(row.prixVenteUnitaire, data.devise)) + "</td><td>" + escapeHtml(formatMoney(row.margeUnitaire, data.devise)) + "</td><td>" + escapeHtml(formatMoney(row.margeTotale, data.devise)) + "</td><td>" + escapeHtml(row.tauxMarge + "%") + "</td></tr>").join("");
    popup.document.write("<!doctype html><html lang='fr'><head><meta charset='utf-8'><title>Projection produits</title><style>@page{size:A4 landscape;margin:12mm}body{font-family:Arial,sans-serif;color:#172033;margin:0}h1{font-size:20px;margin:0 0 4px}p{font-size:11px;color:#64748b;margin:0 0 18px}table{width:100%;border-collapse:collapse;font-size:9px}th{background:#f1f5f9;text-align:left;text-transform:uppercase;color:#64748b}th,td{padding:7px;border:1px solid #e2e8f0;vertical-align:top}.footer{margin-top:12px;font-size:9px;color:#94a3b8}</style></head><body><h1>Projection produits</h1><p>Export du " + escapeHtml(new Date().toLocaleString("fr-FR")) + "</p><table><thead><tr><th>Nom</th><th>Categorie</th><th>Quantite actuelle</th><th>Prix achat unitaire</th><th>Prix vente unitaire</th><th>Marge unitaire</th><th>Marge totale</th><th>Taux</th></tr></thead><tbody>" + rows + "</tbody></table><div class='footer'>StockMaster Pro - Document genere automatiquement</div><script>window.onload=()=>window.print();</script></body></html>");
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            <MetricCard label="Produits analyses" value={String(data.metrics.produitsAnalyses)} detail={String(data.metrics.categoriesAnalysees) + " categorie(s)"} icon={PackageSearch} tone="indigo" />
            <MetricCard label="Quantite actuelle" value={String(data.metrics.quantiteActuelle)} detail="Stock actuel cumule" icon={Layers3} tone="emerald" />
            <MetricCard label="Marge unitaire moyenne" value={formatMoney(data.metrics.margeUnitaireMoyenne, data.devise)} detail="Moyenne par unite en stock" icon={TrendingUp} tone="amber" />
            <MetricCard label="Marge totale projetee" value={formatMoney(data.metrics.margeTotale, data.devise)} detail={String(data.metrics.tauxMargeMoyen) + "% de taux moyen"} icon={BarChart3} tone={data.metrics.margeTotale >= 0 ? "emerald" : "rose"} />
          </div>
          ...
          <section className="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 flex flex-col lg:flex-row gap-3 lg:items-center justify-between">
              <div><h2 className="text-sm font-bold text-slate-900">Projection par {tab === "products" ? "produit" : "categorie"}</h2><p className="text-[11px] text-slate-400 mt-1">Exemple : achat 10, vente 15, stock 10 donne une marge totale de 50.</p></div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[980px] text-left text-xs">
                <thead className="bg-slate-50 text-[10px] uppercase tracking-wider text-slate-400"><tr><th className="px-5 py-4">Nom</th><th className="px-5 py-4">Categorie</th><th className="px-5 py-4">Quantite actuelle</th><th className="px-5 py-4">Prix achat unitaire</th><th className="px-5 py-4">Prix vente unitaire</th><th className="px-5 py-4">Marge unitaire</th><th className="px-5 py-4">Marge totale</th><th className="px-5 py-4">Taux</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {paginatedRows.map((row, index) => <tr key={row.produit + row.categorie + index} className="hover:bg-slate-50/60"><td className="px-5 py-4"><p className="font-bold text-slate-900">{row.produit}</p><p className="text-[10px] text-slate-400">{row.sku || (row.produits ? String(row.produits) + " produit(s)" : "-")}</p></td><td className="px-5 py-4 text-slate-500">{row.categorie || "-"}</td><td className="px-5 py-4 font-black">{row.quantiteActuelle} {row.unite || ""}</td><td className="px-5 py-4 text-amber-700 font-bold">{formatMoney(row.prixAchatUnitaire, data.devise)}</td><td className="px-5 py-4 font-bold">{formatMoney(row.prixVenteUnitaire, data.devise)}</td><td className={(row.margeUnitaire >= 0 ? "text-emerald-600" : "text-rose-600") + " px-5 py-4 font-black"}>{formatMoney(row.margeUnitaire, data.devise)}</td><td className={(row.margeTotale >= 0 ? "text-emerald-600" : "text-rose-600") + " px-5 py-4 font-black"}>{formatMoney(row.margeTotale, data.devise)}</td><td className="px-5 py-4"><span className="px-2 py-1 rounded-md bg-indigo-50 text-indigo-600 text-[10px] font-bold">{row.tauxMarge}%</span></td></tr>)}
                  {paginatedRows.length === 0 && <tr><td colSpan={8} className="px-5 py-12 text-center text-slate-400">Aucune projection trouvee.</td></tr>}
                </tbody>
              </table>
            </div>
