...
export function CashMetric({ label, value, detail, icon: Icon, tone = "indigo" }: { label: string; value: string; detail: string; icon: LucideIcon; tone?: "indigo" | "emerald" | "amber" | "rose" }) {
  ...
  return <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm min-w-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{label}</p><p className="text-xl sm:text-2xl font-black text-slate-900 mt-2 leading-tight whitespace-normal break-words">{value}</p><p className="text-[11px] text-slate-400 mt-1">{detail}</p></div><div className={`w-10 h-10 rounded-xl border flex items-center justify-center shrink-0 ${tones[tone]}`}><Icon size={18} /></div></div></div>;
}
