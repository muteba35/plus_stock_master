...
export function CashHeader({ title, subtitle, action }: { title: string; subtitle: string; action?: React.ReactNode }) {
  ...
  return <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4"><div><h1 className="text-xl font-bold text-slate-900">{du(title)}</h1><p className="text-xs text-slate-400 font-medium mt-1">{du(subtitle)}</p></div>{action}</div>;
}
...
export function CashMetric({ label, value, detail, icon: Icon, tone = "indigo", onInspect }: { label: string; value: string; detail: string; icon: LucideIcon; tone?: "indigo" | "emerald" | "amber" | "rose"; onInspect?: () => void }) {
  ...
  return <div className="bg-white border border-slate-200/80 rounded-2xl p-5 shadow-sm min-w-0"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{du(label)}</p><p className="text-xl sm:text-2xl font-black text-slate-900 mt-2 leading-tight whitespace-normal break-words">{value}</p><p className="text-[11px] text-slate-400 mt-1">{du(detail)}</p></div><div className="flex flex-col items-end gap-2 shrink-0"><div className={`w-10 h-10 rounded-xl border flex items-center justify-center ${tones[tone]}`}><Icon size={18} /></div>{onInspect && <button type="button" onClick={onInspect} className="w-7 h-7 rounded-lg border border-slate-200 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 text-[11px] font-black" title={du("m0050176f676e")}>{du("mde7d1b721a1e")}</button>}</div></div></div>;
}
