...
export const exportXlsxWorkbook = (fileName: string, sheets: XlsxSheet[]) => {
  ...
  sheets.forEach((sheet) => {
    const data = [sheet.columns, ...sheet.rows].map((row) => row.map(normalizeCell));
    ...
    worksheet["!autofilter"] = { ref: XLSX.utils.encode_range({ s: { r: 0, c: 0 }, e: { r: Math.max(0, data.length - 1), c: Math.max(0, sheet.columns.length - 1) } }) };
