...
export const formatDate = (value?: string) => value ? new Intl.DateTimeFormat(dashboardLocale(), { dateStyle: "medium", timeStyle: "short" }).format(new Date(value)) : "-";
export const compactMoney = (value: number, devise: string) => Math.abs(value || 0) < 1000000 ? formatMoney(value, devise) : new Intl.NumberFormat(dashboardLocale(), { notation: "compact", maximumFractionDigits: 2 }).format(value) + " " + devise.replace(/.*\((.*)\).*/, "$1");
...
export function StateBlock({ loading, error }: { loading: boolean; error: string }) {
  const { ui: du } = useDashboardLanguage();
  if (loading) return <div className="py-20 flex flex-col items-center gap-3 text-slate-400"><Loader2 size={26} className="animate-spin text-indigo-500" /><span className="text-xs font-medium">{du("maf3584717ecc")}</span></div>;
  if (error) return <div className="p-3 rounded-xl border border-rose-100 bg-rose-50 text-xs font-semibold text-rose-700 flex items-center gap-2"><AlertCircle size={15} />{du(error)}</div>;
  return null;
}
